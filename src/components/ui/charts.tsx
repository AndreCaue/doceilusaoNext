    ...
    return (
      ...
      >
        {payload.map((item) => {
          ...
          const itemConfig = getPayloadConfigFromPayload(config, item, key);
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const payloadValue = (item.payload as any)?.value;
          const valuePercent = `${(
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (((item.payload as any)?.percent ?? 0) as number) * 100
          ).toFixed(1)}%`;
          ...
          return (
            <div
              key={item.value}
              className={cn(
                "flex items-center gap-1 rounded-md bg-muted px-3 py-1 [&>svg]:h-2 [&>svg]:w-2 [&>svg]:text-muted-foreground",
              )}
            >
              ...
              {showPercentage && (
                <>
                  <span className="text-base font-bold">{valuePercent}</span>
